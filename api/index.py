...
import time
import uuid
import base64 # To encode audio to send over a network
import logging
import datetime as dt
from pathlib import Path # Just to get the path of the directory for testing purpose
from typing import Dict, Any, List, Tuple, Optional

import requests
from flask import Flask, request, jsonify, Response
from flask_cors import CORS, cross_origin
from flask import stream_with_context # Used for streaming the audio
...
def classify_intent_text(user_text: str) -> str:
    ...
    Classify a natural language query into one of:
      read_email, send_email, schedule_event, calendar_answer, health, general, audio_stream
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        " You MUST classify the user's intent into EXACTLY ONE category.\n\n"

                        " INTENT DEFINITIONS:\n"
                        " - audio_stream: The user wants audio output or speech.\n"
                        "   This includes ANY request to:\n"
                        "   speak, say aloud, read aloud, give audio, play audio, listen,\n"
                        "   hear the response, or listen in real time.\n"
                        "   EVEN IF the request is phrased as a question.\n\n"

                        " - read_email: Reading or summarizing inbox emails.\n"
                        " - send_email: Drafting or sending an email.\n"
                        " - schedule_event: Creating a calendar event.\n"
                        " - calendar_answer: Asking about existing calendar events.\n"
                        " - health: Health or wearable-related questions.\n"
                        " - general: Anything else.\n\n"

                        " IMPORTANT RULES:\n"
                        " - If the user requests audio or speech in ANY form, ALWAYS choose audio_stream.\n"
                        " - Do NOT infer intent from politeness or phrasing.\n"
...
# Language Translation along with conditional text to audio endpoint 
# Made by me: Rahul
# Helper function
def stream_tts_audio(text: str, voice: str):
    """
    Stream WAV audio bytes from OpenAI TTS in real time.
    """
...
# Text extraction exactly what to speak
def get_text_to_speak(user_text: str) -> str:
    """
    Convert a user request into EXACT text that should be spoken.
        ...
        input=[
            {
                "role": "system",
                "content": (
                    "You are a voice assistant text generator.\n"
                    "Your job is to return ONLY the exact text that should be spoken aloud, Or the answer of the user query that exactly to be spoken\n"
                    "Rules:\n"
                    "- Do NOT explain\n"
                    "- Do NOT add prefixes\n"
                    "- Do NOT include quotes\n"
                    "- Do NOT mention that you are speaking\n"
                    "- Return plain text only\n\n"
                    "Examples:\n"
                    "User: Speak the following sentence: Hello world\n"
                    "Output: Hello world\n\n"
                    "User: Can you speak the capital of France?\n"
...
# API Endpoint for audio stream
@app.post("/text/stream-audio")
@cross_origin()
def stream_audio():
    if not client:
        return jerror("OpenAI client not configured", 500)
    ...
    if not user_text:
        return jerror("Missing 'text'", 400)

    try:
        # Step 1: Generating exact speakable text
        speak_text = get_text_to_speak(user_text)

        if not speak_text:
            return jerror("No speakable text generated", 400)

        # Step 2: Stream TTS for that text
        return Response(
            stream_with_context(stream_tts_audio(speak_text, voice)),
...
def translate_text():
    ...
    if not target_language:
        return jerror("Missing 'target_language'", 400)

    try:
        system_prompt = (
            "You are a language translation engine.\n"
