...
// Made by me: Rahul
async function streamPCMFromEndpoint(endpoint, payload) {
  ...
  while (true) {
    ...
    // Ensure even byte length (PCM16 = 2 bytes)
    const usableLength = merged.length - (merged.length % 2);
    leftover = merged.slice(usableLength); // save remainder

    if (usableLength === 0) continue;

    const pcm16 = new Int16Array(merged.buffer, 0, usableLength / 2);
...
async function sendQuery(rawQuery) {
  ...
  try {
    ...
    if (intent === "audio_stream") {
      await streamPCMFromEndpoint(
        `${CONFIG.BASE_URL}/text/stream-audio`,
        {
          text: query,
          voice: "coral",
        }
